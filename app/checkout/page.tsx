...
const CheckoutPage = () => {
  ...
  if (cart.length === 0 && notification.type !== "success") {
    return (
      ...
      >
        <h2 className="text-2xl font-bold mb-4 text-[#4F3523]">{t("checkout.emptyCart")}</h2>
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="lg:w-1/3 bg-[#FAF8F5] border border-[#E5D9CA] shadow-[#4F3523]/10 rounded-2xl p-6 h-fit">
          <h2 className="text-xl font-semibold mb-4 text-[#4F3523]">{t("checkout.orderSummary")}</h2>
          <div className="space-y-3">
            {cart.map((item) => (
              <div key={item.id} className="flex justify-between text-sm text-[#4F3523]/80">
                <span>
      ...
      {notification.type && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-[#4F3523]/80 backdrop-blur-sm">
          <div className="bg-[#FAF8F5] border border-[#E5D9CA] rounded-2xl shadow-2xl max-w-md w-full p-6 text-center">
            ...
            <p className="text-[#4F3523]/70 text-sm mb-6">{notification.message}</p>
            <button
              onClick={() => {
                clearCart();
                setNotification({ type: null, message: "" });
